...
test("encryptSecret round-trips through decryptSecret", () => {
  const encrypted = encryptSecret("sk-test-1234567890", APP_SECRET);
  assert.notEqual(encrypted.ciphertext, "sk-test-1234567890");
  assert.equal(decryptSecret(encrypted, APP_SECRET), "sk-test-1234567890");
});
