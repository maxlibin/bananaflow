...
- `src/lib` is the engine: one adapter per provider under `src/lib/providers/` (OpenAI and Google), the model descriptions (`model-registry.ts`, `model-options.ts`), job tables, finalizers, route-handler factories and the scheduler.
